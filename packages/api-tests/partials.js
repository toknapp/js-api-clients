...
function getPartials(testenv) {
  ...
  const tInspectResponse = (t, response) => {
    ...
    const summary = {
      ...
      response: {
        ...
        body: response.data,
        isTextbody: ('string' == typeof response.data),
      },
    }
    t.inspect(summary);
  };

  const tInspectError = (t, error) => {
    if (error.response) {
      tInspectResponse(t, error.response);
    }
    else {
      t.inspect(error);
    }
  ...
  const tWaitForBalanceUpdate = async (t, api, walletId, assetId, previousBalanceAmount, minutes) => {
    ...
    do {
      ...
      // t.inspect(wallet);
      balance = testenv.getBalanceForAssetId(wallet, assetId);
      // t.inspect(balance);
  ...
  return {
    tInspectResponse, tInspectError, tErrorFail, tGetCachedOrCreateUser, tCreateUser, tEcho, tCreateWallets,
